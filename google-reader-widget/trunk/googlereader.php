...
/*
Changes:
2.0 - Added %date% tag. Added support for custom shared id. Added support for multiple users. Added multiple widget support.
...
function widget_googlereader_init() {
	...
	function widget_googlereader_control() {
		...
		echo '<p style="text-align:right;"><label for="googlereader-use">' . __('Use:', 'widgets') . ' <select style="width:200px;" id="googlereader-use" name="googlereader-use"><option value="file_get_contents" '.($option['use'] == "file_get_contents" ? ' selected="selected" ' : "").'>file_get_contents</option><option value="curl" '.($options['use'] == "curl" ? ' selected="selected" ' : "" ).'>curl</select></label></p>';
		echo '<input type="hidden" id="googlereader-submit" name="googlereader-submit" value="1" />';
		echo '<p style="text-align:right;"><a href="http://ja.meswilson.com/blog/2007/05/26/wordpress-google-reader-widget/">Widget Support</a></p>';
		echo '<p style="text-align:right;"><a href="http://ja.meswilson.com/donate">Help Development - Donate</a></p>';
	}
